...
Meteor.methods({
  ...
  var user = Meteor.user();
    ...
    // We check if the user has already torn off the item
    var indexUser = _.contains(item.interestedUsers, user._id);
    if (indexUser){
      throw Meteor.Error(602, "Hasch schon abgerissen");
    }

    var users = item.interestedUsers;
    users.push(user._id);

    Items.update(itemId, {$set: {interestedUsers: users}}, function(error){
      if(error){
        // display the error to the user
        throw Meteor.Error(502, error.reason);
      }
    });

    Items.update(itemId, {$inc: {tearoffs: -1}}, function(error) {
      if(error) {
        // display the error to the user
